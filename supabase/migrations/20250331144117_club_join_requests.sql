...
-- Function to add members to a club
...
begin
    ...
    -- Add the new member
    insert into public.club_members (club_id, user_id, is_owner)
    values (p_club_id, p_user_id, p_is_owner)
    on conflict (club_id, user_id) 
    do update set is_owner = p_is_owner;
...
as $$
begin
    -- Check if current user is club owner or the member being removed
    if not (
        auth.uid() = p_user_id 
        or exists (
            select 1 
        ...
        raise exception 'Only club owners or the member themselves can remove members';
    end if;
    ...
    if exists (
        ...
        raise exception 'Cannot remove the last owner of a club';
    end if;
